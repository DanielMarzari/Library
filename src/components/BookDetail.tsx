      ...
      <div className="relative bg-zinc-900 border border-zinc-800 rounded-t-2xl sm:rounded-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <div className="relative h-48 bg-gradient-to-b from-zinc-800 to-zinc-900 flex items-center justify-center">
          ...
          <div className="absolute top-3 right-3 flex gap-2">
            <button onClick={() => setEditing(!editing)} className="text-zinc-400 hover:text-zinc-200 bg-black/40 rounded-full w-8 h-8 flex items-center justify-center text-sm">
              ✏️
            </button>
        ...
        <div className="p-6 space-y-5">
          {editing ? (
            <>
              ...
              <div>
                ...
                <div className="grid grid-cols-4 gap-2">
                  <div>
                    ...
                    <input type="number" value={pages} onChange={(e) => setPages(e.target.value)} placeholder="—"
                  ...
                  <div>
                    ...
                    <input type="number" value={introPages} onChange={(e) => setIntroPages(e.target.value)} placeholder="0"
                      className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-600" />
                  </div>
                  <div>
                    <label className="block text-[10px] text-zinc-600 mb-0.5">Start</label>
                    <input type="number" value={startPage} onChange={(e) => setStartPage(e.target.value)} placeholder="1"
                      className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-600" />
                  </div>
                  <div>
                    <label className="block text-[10px] text-zinc-600 mb-0.5">End</label>
                    <input type="number" value={endPage} onChange={(e) => setEndPage(e.target.value)} placeholder="—"
                      className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-600" />
                  </div>
                </div>
              ...
              <div>
                ...
                <input type="text" value={source} onChange={(e) => setSource(e.target.value)} placeholder="Gift from Mom, Library, Amazon..."
                  className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-600" />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="text" value={lcc} onChange={(e) => setLcc(e.target.value)} placeholder="e.g. BT202.0318"
                    className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-600" />
                </div>
                <div>
                  <label className="block text-xs text-zinc-500 mb-1">DDC</label>
                  <input type="text" value={ddc} onChange={(e) => setDdc(e.target.value)} placeholder="e.g. 232'.8--dc20"
                    className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-600" />
                </div>
              </div>
            ...
            <>
              <div>
                ...
                {book.source && (
                  <p className="text-xs text-zinc-600 mt-1">Source: {book.source}</p>
                )}
              ...
              {book.description && (
                <p className="text-sm text-zinc-400 leading-relaxed line-clamp-4">{book.description}</p>
              )}
            </>
          )}
              ...
              {(Object.keys(statusLabels) as Book["status"][]).map((s) => (
                <button key={s} onClick={() => handleStatusChange(s)}
                  className={`px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${
                    status === s ? "bg-emerald-600 text-white" : "bg-zinc-800 text-zinc-400 hover:text-zinc-200"
                  }`}>
                  {statusLabels[s]}
                </button>
              ))}
          ...
          <div>
            ...
            <textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Your thoughts on this book..." rows={3}
            ...
            <div>
              ...
              {showAddUpdate && (
                <div className="bg-zinc-800 rounded-lg p-3 mb-3 space-y-2">
                  <input type="number" value={currentPage} onChange={(e) => setCurrentPage(e.target.value)} placeholder="Current page #"
