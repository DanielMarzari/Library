      ...
      <div className="relative bg-surface border border-border-custom rounded-t-2xl sm:rounded-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <div className="relative h-48 bg-gradient-to-b from-surface-2 to-surface flex items-center justify-center">
          ...
          <div className="absolute top-3 right-3 grid grid-cols-3 gap-1.5">
            ...
            <button onClick={handleDelete} className="text-muted hover:text-red-400 bg-black/40 backdrop-blur-sm rounded-full w-8 h-8 flex items-center justify-center" title="Delete book">
          ...
          {status !== "read" && (displayPages || book.pages) ? (
            <div>
              ...
              {(() => {
                ...
                const ip = book.intro_pages || parseInt(introPages) || 0;
                const sp = book.start_page || parseInt(startPage) || 1;
                // Pages actually read = intro pages + (current page - start page + 1)
                const mainPagesRead = currentPg > 0 ? Math.max(currentPg - sp + 1, 0) : 0;
                const actualPagesRead = mainPagesRead + (currentPg > 0 ? ip : 0);
                const pct = Math.min((actualPagesRead / totalPgs) * 100, 100);
                return (
                  <div className="mb-3">
                    ...
                    <div className="flex items-center justify-between mt-1.5">
                      <span className="text-xs text-muted">
                        {ip > 0 && currentPg > 0
                          ? `p. ${currentPg} (+${ip} intro) · ${actualPagesRead} of ${totalPgs}`
